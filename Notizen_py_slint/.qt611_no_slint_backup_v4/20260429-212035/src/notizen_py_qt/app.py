...
def _format_qt_load_error(exc: Exception, ui_path: Path) -> str:
    """Return readable Qt diagnostics.

    Older and current Qt Python bindings often expose diagnostics as opaque
    ...
    lines = [f"Qt konnte die UI-Datei nicht kompilieren: {ui_path}"]
    ...
    if not diagnostics:
        lines.append(f"  {exc}")
    lines.append("Tipp: Die Datei liegt unter src/notizen_py_qt/ui/app-window.qml.")
    return "\n".join(lines)


class NotizenQtApp:
    """Thin controller that binds the pure Python document model to Qt."""

    def __init__(self, initial_path: str | Path | None = None, password: str | None = None) -> None:
        import qt  # imported lazily so CLI/tests can run without starting the GUI

        self.qml = qt
        ui_path = resources.files("notizen_py_qt.ui").joinpath("app-window.qml")
        try:
            components = qt.load_file(str(ui_path))
        except Exception as exc:  # noqa: BLE001 - Qt uses its own CompileError type
            if exc.__class__.__name__ == "CompileError":
                raise RuntimeError(_format_qt_load_error(exc, ui_path)) from exc
            raise
    ...
    # Window actions -------------------------------------------------------
    def _native_qt_window(self) -> object | None:
        """Return the underlying Qt window handle when the binding exposes it."""
    ...
    def toggle_maximized(self) -> None:
        """Best-effort maximize/unmaximize for Qt Python releases/backends."""

        native = self._native_qt_window()
        try:
            ...
            self._set_status(f"Maximieren wird von diesem Qt-Backend nicht unterstützt: {exc}")
            return
        self._set_status("Dieses Qt-Python-Backend bietet keine Maximize-API; nutze den nativen Fensterrand.")

    def toggle_fullscreen(self) -> None:
        """Best-effort fullscreen without relying on a Qt language property.

        Some installed Qt compilers reject the Window property ``full-screen``
        even though newer documentation lists it. Keep the .qml file portable
        ...
        try:
            ...
            self._set_status(f"Vollbild wird von diesem Qt-Backend nicht unterstützt: {exc}")
            return
        setattr(self.window, "fullscreen_enabled", False)
        self._set_status("Vollbild wird von diesem Qt-Python-Backend nicht unterstützt; starte alternativ mit --fullscreen oder nutze Max/Fensterrand.")
    ...
    def _refresh_tree_only(self) -> None:
        ...
        self.window.rows = self.qml.ListModel(rows)
    ...
    def _refresh_title_dirty(self) -> None:
        ...
        self.window.window_title = f"Notizen Py Qt - {name}{locked}{marker}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Notizen.NET-Port für Python mit Qt")
    ...
    parser.add_argument("--fullscreen", action="store_true", help="Vollbild beim Start über QT_FULLSCREEN anfordern")
    ...
    if args.fullscreen:
        # Supported even by some Qt releases that reject the .qml Window property.
        os.environ.setdefault("QT_FULLSCREEN", "1")

    try:
        app = NotizenQtApp(initial_path=args.file, password=args.password)
        if args.fullscreen:
            try:
        ...
        if exc.name == "qt":
            print("Qt ist nicht installiert. Installiere die UI-Abhängigkeit mit:")
            print('  python3 -m pip install -e ".[qt]"')
            return 2
        raise
    except NotizenFileError as exc:
