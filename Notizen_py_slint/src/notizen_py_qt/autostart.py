...
def default_autostart_path() -> Path | None:
    if os.name == "nt":
        ...
        return startup / "Notizen Py Qt.cmd"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "LaunchAgents" / "net.notizen-py-qt.plist"
    return Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "autostart" / "notizen-py-qt.desktop"
...
def _default_command(config: AppConfig) -> list[str]:
    command = [sys.executable, "-m", "notizen_py_qt"]
...
<plist version="1.0">
<dict>
    <key>Label</key><string>net.notizen-py-qt</string>
