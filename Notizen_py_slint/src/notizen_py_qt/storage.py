...
def export_html(document: NoteDocument, path: str | Path, *, start: Note | None = None, title: str | None = None) -> Path:
    ...
    the Python/Qt port a portable preview/print target without extra packages.
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def apply_toolbar_style_to_note(
    ...
    *,
    ...
    RichTextBox selection, or to the whole note when nothing was selected. Qt's
...
def sticky_manifest(document: NoteDocument, *, visible_only: bool = True) -> list[dict[str, object]]:
    ...
    stored in the XML. Qt cannot recreate the WinForms floating windows without
...
def combine_subtree_to_new_note(
    ...
    *,
    ...
    Qt-friendly way. The original built a temporary RichTextBox and pasted all
...
def load_note_json(path: str | Path) -> Note:
    ...
    if isinstance(data, dict) and data.get("format") == "notizen-py-qt-subtree":
        data = data.get("note")
    if not isinstance(data, dict):
...
def insert_text_into_note(note: Note, text: str, at: int) -> Note:
    ...
    result is intentionally rewritten as simple RTF because Qt's editor has no
