...
@dataclass(slots=True, frozen=True)
class RtfStyle:
    """Whole-note style used when Qt cannot address a text selection."""
...
class TextRange:
    ...
    WinForms RichTextBox actions were usually selection based.  Qt's Python
...
def text_to_rtf(
    ...
    *,
    ...
    The old application stored each node as RichTextBox.Rtf. Qt's TextEdit is
...
def detect_rtf_style(rtf_or_text: str, *, default_family: str = "Sans Serif", default_half_points: int = 18) -> RtfStyle:
    ...
    the Python/Qt port's whole-note formatting model and gives toolbar actions
...
def change_rtf_font_size(
    ...
    *,
    ...
    Ctrl+Minus. Qt's plain TextEdit cannot do selection-level rich text, so the
...
def extract_pictures(rtf: str) -> list[RtfPicture]:
    ...
    RichTextBox stores pasted images as RTF picture groups. Qt cannot render or
